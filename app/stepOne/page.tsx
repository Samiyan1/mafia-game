...
export default function Page() {
  ...
  return (
    ...
    <main className="h-screen flex flex-col items-center">
      ...
      <div className="input-container flex w-[80vw]">
        <input required value={name} onChange={(e) => setName(e.target.value)} type="text" placeholder="Mehdi" />
        <button onClick={addPlayers} className="button">Add</button>
      </div>
          ...
          {playerState && playerState.map((item: any, index: number) => {
            return (
              ...
              <div className="InputContainer" key={index}>
                <input value={item} disabled type="text" name="text" className="input font-mono"  />
              ...
              // <div style={{ backgroundColor: '#321E1E' }} className="color black mb-3" >
              //   {item}
              //   <span className="hex"> <button >remove</button></span>
              // </div>
            )
          })}
        </div>
      ...
      <button onClick={() => { playerState.length >= 4 ? router.push('stepTwo', { scroll: false }) : toast.error('تعداد کمتر از حد مجاز') }} type="button" className="btn-submit mt-9">
      ...
      <Image
        src="/logoWhite.png"
        alt="Vercel Logo"
        className="dark:invert mt-[100px] logo z-20 pb-9 "
        width={100}
        height={100}
        priority
      />
